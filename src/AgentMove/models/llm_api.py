...
def get_api_key(platform, model_name=None):
    ...
    elif platform=="Airforce":
        return os.environ["AIRFORCE_API_KEY"]
    elif platform=="Ollama":
        # The OpenAI SDK requires a non-empty value, but Ollama's localhost
        # endpoint does not authenticate by default.
...
class LLMAPI:
    def __init__(self, model_name, platform=None):
        ...
        self.platform_list = ["SiliconFlow", "OpenAI", "DeepInfra", 'vllm', "OpenRouter", "Airforce", "Ollama"]
        self.model_platforms = {
                    ...
                    ,"Airforce":     ['deepseek-v3', 'gemma3-270m-free']
                    ,"Ollama":       ['qwen2:7b', 'qwen2.5-coder:7b', 'qwen2.5:7b', 'qwen2.5:14b', 'llama3.1:8b', 'llama3.2:3b', 'mistral:7b', 'gemma3:4b', 'gemma3:12b', 'deepseek-r1:7b']
                }
        ...
        self.model_mapper = {
            ...
            ,'gemma3-270m-free': 'gemma3-270m:free'
        ...
        elif self.platform == "Airforce":
            self.client = OpenAI(
                base_url=os.environ.get("AIRFORCE_BASE_URL", "https://api.airforce/v1"),
                api_key=get_api_key(platform)
            )
...
class LLMWrapper:
    ...
    def get_response(self, prompt_text):
        ...
        # A conservative local estimate avoids network-dependent tokenizer
        # downloads before every API call.
        if max(1, (len(prompt_text) + 3) // 4) > self.hyperparams['max_input_tokens']:
        ...
        self.__class__._call_stats.append({
            "input_tokens": int(getattr(usage, "prompt_tokens", 0) or max(1, len(prompt_text) // 4)),
            "output_tokens": int(getattr(usage, "completion_tokens", 0) or max(1, len(full_text) // 4)),
