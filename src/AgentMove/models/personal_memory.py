...
class Memory:
    ...
    def write_memory(self, known_stays, context_stays):
        ...
        # pandas 2.2+ may exclude the grouping column from ``groupby.apply``,
        # which made the serialized records lose ``Hour``. Sorting followed by
        # one row per hour is deterministic and stable across pandas versions.
        hourly_venue_summary = (
            hourly_venue_counts
            .sort_values(['Hour', 'Count', 'Venue_Category_Name'], ascending=[True, False, True])
