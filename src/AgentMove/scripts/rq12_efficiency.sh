...
ROOT="results/beliefmove-evo/artifacts/$SCOPE/$CITY/rq12"; FULL_ROOT="results/beliefmove-evo/artifacts/full/$CITY"
...
audit() {
  ...
  echo "city=$CITY seed=$SEED scope=$SCOPE output=$ROOT"
  echo "batch=${BATCH_SIZE:-256} repeats=${BENCHMARK_REPEATS:-5} warmup=${WARMUP_BATCHES:-10} device=${DEVICE:-auto}"
}
run_one() {
  ...
  local extra=(--batch-size "${BATCH_SIZE:-256}" --warmup-batches "${WARMUP_BATCHES:-10}"
...
status() {
  ...
  for seed in ${RQ12_SEEDS:-42 43 44}; do
    for variant in teacher-gru teacher-transformer student-none student-gru student-transformer; do
      path="results/beliefmove-evo/artifacts/full/$CITY/rq12/neural/$variant/seed-$seed/rq12.metrics.json"
      [[ -f "$path" ]] && echo "ready   $path" || { echo "missing $path"; missing=$((missing + 1)); }
    done
    for variant in B0-static B3-dbn; do
      path="results/beliefmove-evo/artifacts/full/$CITY/rq12/bayesian/$variant/seed-$seed/rq12.metrics.json"
      [[ -f "$path" ]] && echo "ready   $path" || { echo "missing $path"; missing=$((missing + 1)); }
    done
  done
...
aggregate() {
  ...
  "$PYTHON_BIN" -m hybrid.rq12_aggregate --root "results/beliefmove-evo/artifacts/full/$CITY/rq12" \
    ...
    --output results/beliefmove-evo/aggregated/rq12_summary.json --markdown ../../ideas/results_rq12.md
}
case "$ACTION" in
  ...
  *) echo "Usage: $0 <audit|status|benchmark-neural|benchmark-bayesian|run-seeds|aggregate>" >&2; exit 2 ;;
esac
