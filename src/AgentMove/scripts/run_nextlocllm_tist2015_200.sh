...
run_city() { local city="$1" base="data/hybrid/TIST2015/$1" out="results/nextlocllm/TIST2015/$MODEL/limit-$LIMIT/$1" file;
  ...
  "$PYTHON_BIN" -m hybrid.nextlocllm_baseline run --dataset TIST2015 --city "$city" --train-csv "$base/getnext/train.csv" --validation-csv "$base/getnext/val.csv" --test-csv "$base/getnext/test.csv" --candidate-ids "$base/candidate_ids.json" \
    ...
    --epochs "$EPOCHS" --batch-size "$BATCH_SIZE" --test-limit "$LIMIT" --train-limit "${TRAIN_LIMIT:-0}" --validation-limit "${VALIDATION_LIMIT:-0}" --device "${DEVICE:-auto}"; }
case "${1:-pending}" in
  pending) for city in "${CITIES[@]}"; do run_city "$city"; done ;;
  audit) for city in "${CITIES[@]}"; do [[ -f "results/nextlocllm/TIST2015/$MODEL/limit-$LIMIT/$city/metrics.json" ]] && echo "done $city" || echo "pending $city"; done ;;
  aggregate) "$PYTHON_BIN" -m hybrid.nextlocllm_baseline aggregate --root "results/nextlocllm/TIST2015/$MODEL/limit-$LIMIT" --cities "${CITIES[@]}" --output "results/nextlocllm/TIST2015/$MODEL/limit-$LIMIT/macro_average.json" ;;
  *) run_city "$1" ;;
esac
