...
class Agent:
    ...
    def predict(self, user_id, traj_id, traj_seqs, target_stay, true_value, stay_points):
        if "agent" in self.prompt_type:
            ...
            # LLM-only prompts do not consume AgentMove's spatial, social, or
            # personal-memory modules; avoid hidden calls that would bias cost.
            spatial_world_info, memory_info, social_world_info = "", {}, ""
        ...
        predictions = {
            ...
            'reason': reason,
            'true': true_venue,
            'call_stats': LLMWrapper.get_call_stats(),
        }
...
class Agents:
    ...
    def single_prediction(self, traj, stay_points, shared_dict=None):
        ...
        LLMWrapper.reset_call_stats()
        # SpatialWorld makes two model calls and is part of AgentMove only.
        spaital_world = None
        if "agent" in self.prompt_type:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--platform', type=str, default="SiliconFlow", choices=["SiliconFlow", "OpenAI", "DeepInfra", "vllm", "OpenRouter", "Airforce", "Ollama"])
