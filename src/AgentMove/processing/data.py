...
class Dataset:
    ...
    def get_dataset(self):
        ...
        if self.dataset_name in ["Shanghai", "Shanghai_Weibo", "Shanghai_ISP"]:
            data['datetime'] = pd.to_datetime(data['utc_time'])
        else:
            # TIST2015 timestamps use e.g. "Tue Apr 03 18:17:18 +0000 2012".
            # An explicit format is both deterministic and much faster than
            # pandas/dateutil inference on million-row cities such as Tokyo.
            data['datetime'] = pd.to_datetime(
                data['utc_time'], format='%a %b %d %H:%M:%S %z %Y', utc=True
            )
    ...
    def get_trajectories(self):
        ...
        if self.trajectory_mode == 'user_split':
            ...
            def _group_by_time_window(data):
                # pandas 3.x excludes grouping columns from GroupBy.apply's
                # frame. Restore the group key explicitly so downstream code
                # can still address user_id after reset_index(drop=True).
                grouped_user_id = data.name
                # Sort by timestamp
