...
def download_data(data_name="www19", use_proxy=True):
    ...
    dataset_urls_dict = {
        ...
        "www2019": 'https://github.com/tsinghua-fib-lab/AgentMove/raw/refs/heads/main/data/www2019_isp_data.zip',
    ...
    if data_name=="tsmc2014" and not os.path.exists(os.path.join(TSMC2014_DATA_DIR, 'dataset_TSMC2014_NYC.txt')):
        ...
        archive = Path(DATA_PATH) / "www2019_isp_data.zip"
        archive.parent.mkdir(parents=True, exist_ok=True)
        if not archive.is_file():
            print(f'Downloading WWW2019 archive to {archive}...')
            with requests.get(dataset_url, stream=True, timeout=180) as response:
                response.raise_for_status()
                total = int(response.headers.get('content-length', 0)) or None
                with archive.open('wb') as handle, _progress_bar(
            ...
            print(f'Using existing WWW2019 archive: {archive}')

        destination = Path(WWW2019_DATA_DIR)
        destination.mkdir(parents=True, exist_ok=True)
        print('Extracting the WWW2019 archive...')
        with zipfile.ZipFile(archive) as bundle:
            corrupt_member = bundle.testzip()
            if corrupt_member:
                raise zipfile.BadZipFile(
                    f"CRC failure in {corrupt_member}; remove {archive} and retry"
                )
            bundle.extractall(destination)
        for nested_name in ("isp.zip", "poi.txt.zip"):
            nested = destination / nested_name
            if not nested.is_file():
                raise FileNotFoundError(
                    f"WWW2019 archive does not contain expected file: {nested_name}"
                )
            with zipfile.ZipFile(nested) as bundle:
                corrupt_member = bundle.testzip()
                if corrupt_member:
                    raise zipfile.BadZipFile(f"CRC failure in {nested_name}:{corrupt_member}")
                bundle.extractall(destination)
            nested.unlink()
        required_www2019 = (destination / "isp", destination / "weibo", destination / "poi.txt")
        missing_www2019 = [str(path) for path in required_www2019 if not path.is_file()]
        if missing_www2019:
            raise FileNotFoundError(
                "WWW2019 extraction incomplete; missing: " + ", ".join(missing_www2019)
            )
        print('Extraction complete!')
