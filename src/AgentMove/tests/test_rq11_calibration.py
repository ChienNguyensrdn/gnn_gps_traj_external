...
from hybrid.rq11_calibration import (arrays_from_probabilities, fit_temperature,
...
class RQ11CalibrationTests(unittest.TestCase):
    ...
    def test_temperature_is_selected_from_validation_grid(self):
        ...
        selected = fit_temperature(iter(batches), [0.5, 1.0, 2.0, 4.0])
    ...
    def test_streamed_test_outputs_keep_only_scalar_arrays(self):
        ...
        before, after = collect_test_outputs(batches, 2.0)
        self.assertEqual(set(before), set(after))
        self.assertEqual(before["labels"].shape, (2,))
        self.assertNotIn("probabilities", before)
