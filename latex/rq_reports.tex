...
\subsection{RQ1 --- Baseline reproducibility}
...
\subsubsection*{Quantitative baselines --- Tokyo matched full-test}
...
\paragraph{Phân tích quantitative baseline}
...
\subsubsection*{TIST2015 bounded baseline --- 12 thành phố}
...
\paragraph{Phân tích bounded baseline}
...
\subsubsection*{Quan hệ giữa hai protocol}
...
\subsubsection*{Publication gate và giới hạn}
...
\subsubsection*{Cập nhật phạm vi 12 thành phố}
...
\subsection{RQ2 --- Bayesian student data-only}
...
\paragraph{DBN là baseline data-only tốt nhất về ranking}
...
\paragraph{Teacher vẫn vượt toàn bộ baseline data-only}
...
\paragraph{BN và Markov thể hiện các ưu thế khác nhau theo cutoff}
...
\paragraph{Không dùng ECE thấp của unigram để kết luận mô hình tốt}
...
\paragraph{Xác suất data-only cần được hiệu chỉnh tốt hơn}
...
\subsubsection*{Protocol gate và giới hạn}
...
\subsubsection*{Kết quả mở rộng TIST2015 --- 12 thành phố}
...
\subsection{RQ3 --- LLM knowledge distillation}
...
\paragraph{Quantitative teacher là nguồn gain chính}
...
\paragraph{LLM-only có tín hiệu dương nhưng chưa đạt significance}
...
Theo tiêu chí đã định nghĩa trong \path{ideas/pipline.md}, kết quả hiện tại chưa đủ để khẳng định M2 tốt hơn M1. Cần tăng số query, mở rộng thành phố hoặc cải thiện structured evidence trước khi đưa ra claim này.

\paragraph{M4 tốt nhất ở R@1/MRR nhưng chưa chứng minh gain bổ sung của LLM}
...
\paragraph{M4 vượt M2 chủ yếu phản ánh quantitative teacher}
...
\paragraph{Calibration không đi cùng ranking}
...
\subsubsection*{Kết quả mở rộng TIST2015 --- 12 thành phố (bounded)}
...
\subsection{RQ4 -- BeliefMove-Evo --- Kết quả thực nghiệm}
...
\subsubsection*{Kết quả test chính}
...
\subsubsection*{Khoảng tin cậy 95\% trên test}
...
\subsubsection*{Kết quả validation}
...
\subsubsection*{Phân tích RQ4}

\paragraph{Đóng góp chính đến từ knowledge distillation}
...
\paragraph{Trajectory hoặc velocity riêng lẻ chưa tạo thêm lợi ích}
...
\paragraph{E5 đạt mean tốt nhất trên toàn bộ test metrics}
...
\subsubsection*{Kết luận có thể dùng trong bài báo}
...
\subsubsection*{Publication gate}
...
\subsection{RQ5 --- Ảnh hưởng của thứ tự trajectory}
...
\subsubsection*{Phạm vi 12 thành phố}

Macro correct đạt R@1/R@5/R@10/MRR \path{0,171903/0,327474/0,384337/0,245829}; reverse đạt \path{0,163801/0,323969/0,380686/0,239358}; random đạt \path{0,164812/0,324336/0,381848/0,240639}. Correct đứng đầu cả bốn metric. Paired significance Tokyo không tự động áp dụng cho macro; cần kiểm định phân tầng theo city nếu muốn claim significance đa thành phố.


\subsection{RQ6 --- Dual-Axis Evolution}
...
E5 đạt R@1 \texttt{0,171903}, R@5 \texttt{0,327474}, R@10 \texttt{0,384337}, MRR \texttt{0,245829}. E6-temporal đạt tương ứng \path{0,168992/0,325905/0,382047/0,243706} và NLL \texttt{6,671989}, thấp hơn nhẹ E5 (\texttt{6,673658}). Summary macro không xuất CKA, cosine, trajectory-bin hoặc paired test đa thành phố; kết luận cơ chế vẫn phải đọc cùng phân tích Tokyo.


\subsection{RQ7 --- Phân tích Belief Memory}
...
Thực nghiệm được tiến hành trên \textbf{TIST2015--Tokyo}, sử dụng checkpoint \path{E5-dual/correct} đã đóng băng với ba seed \texttt{42, 43, 44}. Mỗi trajectory được đánh giá trên toàn bộ prefix theo thứ tự thời gian. Belief được reset khi bắt đầu trajectory mới.
...
\subsubsection*{Kết quả tổng hợp trên test}
...
\paragraph{History và sequential belief không vượt baseline}
...
\paragraph{DBN cải thiện rõ rệt chất lượng xếp hạng}
...
\paragraph{DBN tạo ra trade-off calibration}
...
\subsubsection*{Giới hạn và công việc tiếp theo}
...
\subsubsection*{Trạng thái publication gate}
...
\subsection{RQ8 --- Phân tích Uncertainty-aware LLM Routing}

\subsubsection*{Câu hỏi nghiên cứu và protocol}
...
\subsubsection*{Kết quả tại primary budget}
...
\subsubsection*{Threshold được chọn trên validation}
...
Giá trị entropy tuyệt đối khá lớn vì phân phối trải trên candidate space của Tokyo. Khi mở rộng nhiều thành phố có số POI khác nhau, nên dùng normalized entropy \path{H(p)/log(K)} hoặc fit threshold riêng cho từng city.

\subsubsection*{Phân tích quality và chi phí}

\paragraph{Always-LLM không cải thiện nhất quán}
...
\paragraph{Entropy tiết kiệm chi phí nhưng không tăng chất lượng}
...
\paragraph{Random control tốt hơn Entropy tại Recall@5}
...
\subsubsection*{Oracle-gain upper bound}
...
\subsubsection*{Tóm tắt paired significance}
...
\subsection{RQ9 --- Phân tích Semantic Knowledge Verification}
...
\subsubsection*{Kết quả paired significance ban đầu}
...
\subsubsection*{Invalid-output confound}
...
\subsubsection*{Jointly-valid paired analysis}
...
\subsubsection*{Kiểm định invalid-output rate}
...
\subsubsection*{Ranking sensitivity}
...
\subsection{RQ10 --- Độ bền theo kiến trúc teacher}
...
\subsubsection*{Chất lượng teacher trên test}
...
\subsubsection*{Student sau distillation trên test}
...
\paragraph{So với student không distillation}
...
\paragraph{GRU teacher so với Transformer teacher}
...
\subsubsection*{Paired significance}
...
\subsubsection*{Phạm vi và hạn chế}
...
\subsection{RQ11 --- Calibration đa mục tiêu}
...
\subsubsection*{Distillation --- last-query}
...
Reliability diagram identity--ECE: \path{results/beliefmove-evo/aggregated/rq11_distillation_reliability.svg}.
...
Reliability diagram identity--ECE: \path{results/beliefmove-evo/aggregated/rq11_bayesian_reliability.svg}.
...
\subsubsection*{Paired NLL/Brier tests}
...
\subsubsection*{Bootstrap ECE tests}
...
\subsubsection*{Snapshot 12 thành phố hiện có}
...
\subsection{RQ12 --- Đánh đổi giữa độ chính xác và hiệu quả}
...
\subsubsection*{Batch-1 --- độ trễ single-request}

\paragraph{Neural --- last-query}
...
\subsubsection*{Batch-256 --- throughput}
...
\subsubsection*{LLM routing --- bounded Tokyo limit=200}
...
\paragraph{Hiệu quả của distillation}
...
\paragraph{Chi phí của Bayesian belief}
...
Nút thắt chính không nằm ở neural forward mà ở \path{Post-processing/Fusion}: với batch-256, B3-dbn dùng khoảng 811,680 giây qua năm repeat, so với 181,453 giây của B0-static. Vì vậy, nếu triển khai production, tối ưu vector hóa hoặc chuyển Bayesian fusion khỏi vòng lặp Python là ưu tiên quan trọng.

\paragraph{Chi phí của LLM}
...
\subsection{RQ13 --- Robustness với đầu vào thiếu hoặc nhiễu}
...
\paragraph{Clean so với perturbation}
...
Mỗi dòng trên có bootstrap 95\% CI không cắt 0 và Holm-adjusted p < 0,05 cho các metrics được ghi. Bảng đầy đủ theo từng metric được lưu trong \path{results/beliefmove-evo/aggregated/rq13_summary.json}.

\paragraph{Dose-response trực tiếp}
...
\paragraph{Mô hình tương đối bền với thiếu một phần GPS trajectory}
...
\paragraph{Timestamp noise ảnh hưởng ranking tổng thể nhiều hơn Recall@K}
...
\paragraph{Position noise là failure mode nghiêm trọng}
...
\paragraph{User context quan trọng hơn temporal context}
...
\paragraph{Không diễn giải ECE tách khỏi accuracy, NLL và Brier}
